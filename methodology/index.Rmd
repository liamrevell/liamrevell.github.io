...
consists of applications that are designed to *estimate* the true number of COVID-19 infections over the course of the pandemic. For
...
margin. Since there are a variety of reasons that the *true* number of infections (rather than simply the number of confirmed cases) is
of interest, these applications are designed to help users apply a model to estimate the daily number of new infections, the plausible
range of new infections, the cumulative number of infections, or the daily or cumulative infections as a percentage or per 1M population.

Each of these applications uses a model - but it's one that must be *parameterized* by the user. Users parameterize the model by specifying a
...
reasonable numbers, as detailed in the sections below, but users are nonetheless strongly encouraged to apply multiple values and examine the 
...
given 50 COVID-19 deaths on day *i* + *k*, and an IFR of 0.5%, we would predict that 10,000 new SARS-CoV-2 infections had occurred 
on day *i*.
