...
infections between now - *k* and the present day. For this, we assumed a [sigmoidal](https://en.wikipedia.org/wiki/Sigmoid_function) 
...
symptomatic, it is unlikely to rise to near 1.0. The plot below shows daily estimated infections / confirmed cases for all U.S. data 
over the entire course of the pandemic. Although the fit of the sigmoid function is relatively good, readers might note that the 
fitted curve *overestimates* the ratio of confirmed cases / infections near the start of the pandemic (i.e., to the left-hand side of
the plot). This is not particularly worrisome because the most important part of the graph is the *right* side - since this is the 
part that will be use to extrapolate recent infection numbers from observed cases.
...
Figure 1 seems to show a ratio of confirmed / estimated infections of about 0.45; however, the reader should keep in mind that in
practice this is estimated separately for each jurisdiction.
...
To obtain estimated infections for these days, we divided our observed cases (in recent days) by our fitted values from the 
sigmoid curve.
...
level. Our dataset did *not* include a separate population size estimate for Puerto Rico - so I used the values from 
...
immunity'](https://en.wikipedia.org/wiki/Herd_immunity). The following graph shows estimated infections as a fraction of the total 
population for the U.S. state of Texas, using the same IFR model as in Figures 1 and 2.

```{r, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width=12, fig.height=10, units="in", dpi=150, fig.cap="**Figure 3**: a) Observed daily COVID-19 deaths and an assumed model of IFR. b) Estimated daily infections (green), cases (blue), and deaths (red)."}
infection.estimator(data=Data,
	state="Texas",
	ifr=c(0.015,0.01,0.007,0.006,0.006,0.006,0.006),
	las=1,cex.axis=0.8,cex.lab=0.9,
...
This model is very simple. We merely assume that if we *knew* the IFR for our population of interest on day *i* then we could
*predict* the number of deaths on day *i* + *k*, in which *k* is the lag-time from infection to death (for SARS-CoV-2 infections
leading to death). Having observed the deaths, & supposing a particular value of IFR for day *i*, we can likewise *reconstruct*
the most plausible number of infections for that day.
...
IFR for the population of interest. An IFR that is *too high* (overall or at a specific time during the pandemic) will have the 
...
observing 50 COVID-19 deaths an IFR of 0.5% implies that these correspond to a total of 1,000 infections; whereas an IFR of 1.0%
would only imply that 500 infections occurred. Assuming an IFR value that is *too low* will, obviously, have exactly the opposite
effect.
...
mind, of course, that the true IFR has generally [been falling over time](https://doi.org/10.1038/d41586-020-03132-4)).

In estimating the number of daily infections from now - *k* to the present, we *assume* that the relationship between time (since
...
To capture this dynamic, we devised a plotting method in which we'd overlay the daily or cumulative COVID-19 infections under our
model (outlined above), separated by state. We specifically selected a geographic color palette such that 
...
of IFR is the same for all U.S. states. This assumption is quite dubious, as IFR might be expected to *rise* in locations were 
...
overall geographic dynamics of the COVID-19 pandemic in the U.S.

## Computing a plausible range of infection numbers

A relatively simple extension of our infection estimation method, above, is to admit *uncertainty* about the specific value of IFR
at any particular time during the pandemic, and measure the *sensitivity* of our prediction to a range of different values of IFR.
...
This plotting method shares all the assumptions of our infection estimator. It should be noted that although the shaded region around
